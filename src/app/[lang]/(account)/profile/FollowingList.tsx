...
export function FollowingList({ userId, initial, pageSize }: { userId: number; initial: FollowingPage; pageSize: number }) {
  ...
  async function load(q: string, after: number | null) {
    ...
    try {
      ...
      if (id === requestId.current) setError(t('profile.followingFailed'));
    } finally {
  ...
  if (!initial.total) {
    return <p className="surface px-4 py-10 text-center text-subtle">{t('profile.followingNone')}</p>;
  }
    ...
    <div className="space-y-3">
      {initial.total > pageSize ? (
        <input
          ...
          placeholder={t('profile.filterHandle')}
          aria-label={t('profile.filterHandleLabel')}
          className="field"
        />
      ) : null}
      ...
      {shown ? (
        <ul className="surface divide-y divide-line">
          {page.following.map((u) => (
            <li key={u.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <Link href={`/search?q=user:${encodeURIComponent(u.userName.replace(/^@/, ''))}`} className="min-w-0 flex-1 truncate font-semibold text-ink hover:text-link hover:no-underline">
                {u.userName}
              </Link>
        ...
        <p className="surface px-4 py-8 text-center text-subtle">{t('profile.followingNoMatch', { filter: lastFilter.trim() })}</p>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p role="status" className="text-sm text-subtle tabular-nums">
          {loading ? t('common.loading') : page.total > shown || lastFilter.trim() ? t('profile.showingOf', { shown, total: page.total }) : null}
        </p>
        {page.next ? (
