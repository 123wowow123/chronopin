...
import type { BlockedUser } from '@/lib/client/blocks';
...
// The people you blocked (0076), a tab of the profile.
export default async function BlockedPage() {
  const user = await requireViewer('/profile/blocked');
  const [blocked, t] = await Promise.all([UserBlock.list(user.id), getT()]);
  return (
    <div className="mx-auto max-w-2xl px-4 py-6 sm:py-10">
      ...
      <BlockedList initial={toJson<BlockedUser[]>(blocked)} />
    </div>
  );
}
