...
// Trends' daily RSS (trends.google.com/trending/rss, free, no key). Read by
// `npm run trends:discover` and by the daily jobs' google_trends tool
// (src/server/jobs/tools.ts); why it scores rather than blocklists, and why
// it posts nothing itself, is in docs/okf/scraping/nightly-jobs.md#google-trends.

import * as db from '../db';
import log from '../util/log';

export const DEFAULT_GEOS = ['US', 'GB', 'JP', 'DE', 'IN', 'BR', 'AU', 'CA', 'FR', 'KR'];

export type NewsItem = { title: string; url: string; source: string };
export type Trend = { geo: string; term: string; traffic: number; picture: string | null; news: NewsItem[] };
...
export async function fetchGeo(geo: string): Promise<Trend[]> {
  ...
  if (!response.ok) {
    log.warn(`google trends ${geo}: ${response.status}`);
    return [];
  }
...
export function eventScore(trend: Trend): number {
...
// contains "epic".
export async function alreadyCovered(trend: Trend): Promise<number | null> {
...
export type TrendCandidate = Trend & { score: number; coveredByPin: number | null };

// Every geography's feed, scored, de-duplicated across geographies and
// checked against the pins, best first. `all` keeps the same-day noise too.
export async function trendCandidates({ geos = DEFAULT_GEOS, all = false, minTraffic = 0 } = {}): Promise<{ total: number; candidates: TrendCandidate[] }> {
  const read: Trend[] = [];
  for (const geo of geos) read.push(...(await fetchGeo(geo)));
  const kept = read.map((t) => ({ ...t, score: eventScore(t) })).filter((t) => t.traffic >= minTraffic && (all || t.score > 0));
  ...
  const ranked = [...byTerm.values()].sort((a, b) => b.score - a.score || b.traffic - a.traffic);
  const candidates: TrendCandidate[] = [];
  for (const t of ranked) candidates.push({ ...t, coveredByPin: await alreadyCovered(t) });
  return { total: read.length, candidates };
}
