...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    watch: {
      babel: {
        files: ['<%= yeoman.client %>/{app,components,model}/**/!(*.spec|*.mock).js'],
      ...
      scrape: {
        files: ['<%= yeoman.server %>/scrape/src/**/*.js'],
        tasks: ['scrape']
      },
      jsTest: {
        files: ['<%= yeoman.client %>/{app,components,model}/**/*.{spec,mock}.js'],
    ...
    jshint: {
      ...
      server: {
        ...
        src: ['<%= yeoman.server %>/**/!(*.spec|*.integration).js', '!<%= yeoman.server %>/scrape/scrape.min.js', ]
      },
      serverTest: {
        options: {
      ...
      all: ['<%= yeoman.client %>/{app,components,model}/**/!(*.spec|*.mock|app.constant).js'],
      test: {
        src: ['<%= yeoman.client %>/{app,components,model}/**/*.{spec,mock}.js']
      }
    },
    ...
    ngconstant: {
      options: {
        name: 'chronopinNodeApp.constants',
      ...
      dist: {
        files: [{
          ...
          src: [
            ...
            //'node_modules/**/*' //copy node_modules to dist so azure can simply deploy dist folder that contains everything
          ]
        }]
      },
      deploy: {
    ...
    babel: {
      ...
      client: {
        files: [{
          expand: true,
          cwd: '<%= yeoman.client %>',
          src: ['{app,components,model}/**/!(*.spec).js'],
          dest: '.tmp'
        }]
      },
      server: {
    ...
    browserify: {
      scrapeJs: {
        files: {
          'server/scrape/scrape.min.js': [
            '.scrape/build/scrape/src/combine.js'
          ]
        }
      }
    }
  ...
  grunt.registerTask('deploy', [
    //'default',
    'build', // remove this if default is put back
    'clean:deploy',
    'copy:deploy'
  ]);
