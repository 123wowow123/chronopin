...
//
// Vendored as-is apart from one fix, marked CHRONOPIN PATCH in layerPointer().
// Used by components/pin-parallax.
	...
	function layerPointer(elem, parallax, px, offset, size) {
		...
		while (i--) {
			position[i] = px[i] ?
				...
				0 :
				// CHRONOPIN PATCH: was divided by (viewport size - layer size),
				// which is not the inverse of layerCss(). layerCss() sets
				// top: pos * 100% of the viewport and margin-top: -pos * layer
				// size, and jQuery's .position() adds the margin back, so it
				// reports exactly pos * viewport size - measured, not assumed.
				// Dividing by (viewport - layer) made the plugin misread the
				// layer's starting position, which forced the first frame to
				// jump instead of easing out of where the layer actually was.
				pos[i === 0 ? 'left' : 'top'] / viewport[i === 0 ? 'outerWidth' : 'outerHeight']() ;
