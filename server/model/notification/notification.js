...
export default class Notification {
  ...
  static create({ userId, actorId, type, pinId }, query) {
    query = query || db.query;
    return query(`
      INSERT INTO "Notification" ("userId", "actorId", "type", "pinId")
      VALUES ($1, $2, $3, $4)
      RETURNING "id"`, [userId, actorId, type, pinId == null ? null : pinId]);
  }

  // Soft-deletes what an undone action had sent, read or not.
  static retract({ userId, actorId, type }, query) {
    query = query || db.query;
    return query(`
      UPDATE "Notification"
  ...
  static list(userId, limit) {
    limit = Math.min(Math.max(parseInt(limit, 10) || DEFAULT_LIMIT, 1), 100);
    return db.query(`
  ...
  static unreadCount(userId) {
    return db.query(`
      ...
      FROM "Notification" n
      JOIN "User" a ON a."id" = n."actorId" AND a."utcDeletedDateTime" IS NULL
      WHERE n."userId" = $1 AND n."utcReadDateTime" IS NULL AND n."utcDeletedDateTime" IS NULL`, [userId])
  ...
  static markAllRead(userId) {
    return db.query(`
      ...
      WHERE "userId" = $1 AND "utcReadDateTime" IS NULL AND "utcDeletedDateTime" IS NULL`, [userId]);
  }
}
