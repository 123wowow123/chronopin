-- A pin's tags: short free-form metadata, searched with tag: terms, shown in
-- the tag cloud and written as the pin concept's OKF tags (src/lib/tags.ts).
--
-- PinTag holds the ones typed in the form ("user") and the awards its own
-- description and summary name ("auto", rewritten after each save). Award
-- tags are derived: every body and year in PinAward (0036) is a tag, e.g.
-- "Tokyo Anime Award Festival 2024", so they follow the award sync with
-- nothing to keep in step. PinTagView is all of them, one row per pin and
-- name whatever the case, the user's spelling first.
CREATE TABLE "PinTag" (
  "id"                 integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "pinId"              integer NOT NULL REFERENCES "Pin" ("id") ON DELETE CASCADE,
  "name"               citext NOT NULL CONSTRAINT "CK_PinTag_name" CHECK (length("name") BETWEEN 1 AND 80),
  "kind"               varchar(10) NOT NULL CONSTRAINT "CK_PinTag_kind" CHECK ("kind" IN ('award', 'topic')),
  "source"             varchar(10) NOT NULL CONSTRAINT "CK_PinTag_source" CHECK ("source" IN ('user', 'auto')),
  "utcCreatedDateTime" timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT "UC_PinTag" UNIQUE ("pinId", "name")
);
CREATE INDEX "IX_PinTag_name" ON "PinTag" ("name");

CREATE VIEW "PinTagView" AS
...
-- The pin page reads its tags off the view.
