-- Pin cards show each ticker's price now and its move since the pin's start
-- date (src/components/pin/PinCardStocks.tsx). The tickers ride along on the
-- pin as one json array, like "references" and "ratings".
...
SELECT
  ...
  -- Appended last, as CREATE OR REPLACE requires; the rest is 0017's view
  -- unchanged. The pin's tickers (not the ones taken off), company first,
