-- Whether the *source* said the event takes a whole day (or several), as
-- opposed to merely not printing a clock time.
--
-- "allDay" (0001) has always meant both at once, and in practice almost
-- always the second: 2123 of 2239 pins carry it, because a page that dates a
-- signing, a release or a launch to a day and no hour leaves the extractor
-- nothing else to store. That flag is load-bearing - it is what makes the pin
-- a whole UTC day for the timeline, date: search, weather and JSON-LD
-- (CK_Pin_allDayUtcMidnight, 0006) - so it stays exactly as it is.
--
-- This column carries only the claim: true when the page itself describes an
-- event that runs all day or across days (a festival weekend, a sale window,
-- a funding period), false when we simply never learned the time. The pin
-- page prints "All day" only for the true ones, so the label reads as a fact
-- about the event instead of an admission about the data.
--
-- Existing rows default to false: unknown until an author or a re-scrape says
-- otherwise, which is the honest answer for all but a handful of them.
ALTER TABLE "Pin"
  ADD COLUMN "allDayStated" boolean NOT NULL DEFAULT false;

ALTER TABLE "Pin"
  ADD CONSTRAINT "CK_Pin_allDayStated" CHECK (NOT "allDayStated" OR "allDay");
