-- How many episodes a work released in episodes has: a TV series, an anime, a
-- web or podcast series. episodeCount is the number itself; episodeStatus
-- says what it counts, because the same number means different things while a
-- show is still going out:
--   complete  the whole run, which has finished (the default reading)
--   ongoing   the episodes out so far, with more coming and no total announced
--   planned   the total announced for a run that has not finished airing
-- Both are null for a film, a one-off event, or a show whose count is unknown.
-- Filled by the extractor from the page and by the AniList/MyAnimeList/
-- Wikidata lookup (src/server/scrape/screen.ts).
ALTER TABLE "Pin"
  ADD COLUMN "episodeCount" integer,
  ADD COLUMN "episodeStatus" varchar(10);

ALTER TABLE "Pin"
  ADD CONSTRAINT "CK_Pin_episodeCount" CHECK ("episodeCount" IS NULL OR "episodeCount" > 0),
  ADD CONSTRAINT "CK_Pin_episodeStatus" CHECK (
    "episodeStatus" IS NULL OR ("episodeCount" IS NOT NULL AND "episodeStatus" IN ('complete', 'ongoing', 'planned'))
  );
