...
## 2026-09-26 - Video media for the 110 fashion pins, on production

Only the Met Gala 2026 pin (4194) of the fashion batch (4165-4274) carried a video; Ian chose "all 109" over the show/event pins alone. Folder `.scrape/prod-batches/fashion-video-2026-09-26/` (BRIEF.md, `yt.py`, proposals-*.json, `apply.py`, done.jsonl, `before/` snapshots). Five agents proposed one video per pin, then a second pass with a looser `context` bar took the 29 left; the lead re-checked every pick through oEmbed and applied them by a GET -> PUT round trip as the pin's own desk (tags dropped, media `type` cast to a number, the `{type: 3, originalUrl: watch URL}` medium appended). Result: 105 of 110 pins now have a video, each with its still; a before/after diff of all of them showed nothing but the new medium (references come back with new ids, same URLs). Left without: 4195 (held, see below), 4227 Supreme, 4248 Puma/Hoeld, 4202 Primark/Tonge (only off-topic candidates).

* **Learned - `yt.py search` works keyless:** `ytInitialData` from `youtube.com/results?hl=en&search_query=` gives id, title, channel, age, length and views; `check` = oEmbed (401 = embeds off) + `ytInitialPlayerResponse` for upload date, length, embed size (vertical?) and playability. A channel's uploads without a key: `youtube.com/feeds/videos.xml?channel_id=<id>` (the organiser channel's id is often in the pin's sourceUrl page, e.g. FHCM's Paris Fashion Week® channel).
* **Learned - what exists per pin kind.** Runway debuts and fashion weeks: the brand's own full-show upload (Celine, Givenchy, Loewe, Valentino, Versace, Prada, Burberry, Michael Kors). Results: companies rarely post their own, so Reuters/Bloomberg "Stock Movers"/CNBC/Yahoo Finance, and for Inditex, Fast Retailing and H&M the home broadcasters (Europa Press/EFE, ANN/TV Tokyo, EFN). Forward "due to report" pins and completion/closing pins have nothing of their own: they carry the latest results or the deal's announcement coverage, labelled context. Future editions of awards and weeks: the organiser's film of the last edition.
* **Learned - reject:** earnings-call re-upload channels (i101, EARNMOAR, Castify), runway re-uploaders (Fashion Feed, FF Channel) when the brand's own upload exists, podcast episodes where the pin's story is one segment deep in (Prof G), panels about something else (a Puma x Hyrox talk for Puma's CEO).
* **Learned - a 15-20 s official clip beats a longer generic one when it is the pin's own announcement:** 4269 got FHCM's "The Official Paris Fashion Week® Calendar is out!" (17 s, 18 Sep), 4270 Victoria's Secret's "LIVE from LA on Sunday, October 18" teaser. Re-check 4269 after 6 Oct for a recap of the week itself.
* **Learned - a video search surfaces stale pins.** 4195 says the Met Gala 2027 will honour John Galliano, but the Met cancelled "John Galliano: Horizons" on 31 Aug 2026 (the 2027 theme is unannounced); 4241 says Isak Andic died "in a hiking accident", but his death has been investigated as a homicide since Oct 2025 and his son Jonathan was arrested on 19 May 2026. 4216's "shares fall" may be intraday - Reuters says Shein's Hong Kong debut closed flat. Raised with Ian.
* **Changed:** this entry, and the standing feedback above.
...
* **Learned - info.gov.hk is the primary source for any Hong Kong government event.** The day index `/gia/general/<yyyymm>/<dd>.htm` lists every release. Each release ends `Issued at HKT hh:mm`, which dates the event to the minute when nothing else does: the plan's full text went out at 11:12 and the Policy Address at 12:29, which, with HKFP's live blog opening at 03:00Z, gave the 11:00 HKT start. Releases marked "(with photos)" link 4000 px originals at `gia.info.gov.hk/general/<yyyymm>/<dd>/<release id>_photo_<n>.jpg` (the `t` suffix is the thumbnail), captioned in the `fancybox` `title`, and prod fetches them with plain curl.
* **Learned - an opinion link pins the news event it argues about.** The WSJ piece is commentary on the 16 September announcement, so the pin is the announcement, placed at the Legislative Council Complex and timed to the speech.
* **Feedback**: none yet. The owner OK'd the prod post when asked (a bare "pin <url>" is not "on prod").
* **Changed**: [Sources](sources.md) gains rows for wsj.com and info.gov.hk.
