...
| **whitehouse.gov briefings** | `briefings-statements/<yyyy>/<mm>/congressional-bill-h-r-NNNN-signed-into-law/` reads with a plain browser UA and states the weekday, date, bill number, the act's name and what it funds and until when; the **Related** list indexes the neighbouring signings | - | The primary source for the date a funding bill became law, better than any news copy. Walk the Related list to collect a run of signings in one fetch |
