...
| Oracle news | @TechDesk; @CyberDesk breaches, zero-days and patch days, @LawDesk suits and rulings (`company: Oracle`), @EconDesk layoffs, @HealthDesk VA EHR go-lives (company = the VA, ORCL `related`) | the subject's own (`Finance` + `AI`/`Software` for results, `AI` + `Infrastructure` for data centres, `Conference` + `AI` for AI World, `Software` for Java, `Cybersecurity` first for security, `Law` first for courts, `Labour` first for layoffs) | oracle.com/news pages and `news.xml` (Chrome UA), PR Newswire stamps and EDGAR (CIK 1341439) acceptance times, oracle.com/data-centers/ tracker for site delivery halves, OpenAI posts via Wayback, Oracle security alerts + CISA KEV JSON, CourtListener search API / RECAP PDFs, state WARN files | Every pin carries the tag `Oracle`. Results chain oldest first at ~16:05 ET, the next one `estimated` from the 9th/10th pattern until the "Sets the Date" release. Each Stargate site is its own chain ending in an `estimated` delivery half-year; a patch day is `scheduled` from Oracle's third-Tuesday list. Prod's Company note for Oracle came from the layoffs pin - check `tickerNote` before a batch |
| Salesforce news | @TechDesk (breaches and extortion @CyberDesk; lawsuits, rulings and a regulator's clearance @LawDesk; layoffs and hiring @EconDesk) | the subject's own (`Finance` + `Software` for results and capital, `Business` + `Software`/`AI` for deals, `AI` + `Software` for products and partnerships, `Conference` for TDX/Connections/World Tour, `Cybersecurity`, `Law`, `Labour`) | salesforce.com's newsroom WordPress API (`/news/wp-json/wp/v2/sf_press_release`, `posts`, `media`; `date_gmt` is the release minute), EDGAR (CIK 1108524) 8-Ks and 10-Qs for deal prices and close days, Nasdaq's Business Wire copies for results times, the regional event pages for World Tour venues, CourtListener, Washington ESD's WARN form | Every pin carries the shared tag `Salesforce`. Results chain oldest first; the next one is `estimated` for the Wednesday after the fiscal quarter's pattern (Q3 = the week after Thanksgiving) until the timing notice. A deal is agreement -> close, the close day often only an editor's note on the agreement story. A breach touching Salesforce customers takes the principal party as company and CRM as `related`. Leave Dreamforce and SF WARN pins to a San Francisco batch |
...
| Apparel brand news (Vuori) | @FashionDesk for funding, leadership, ambassadors, collections and campaigns; @RetailDesk for each store opening (the store is the pin); @LawDesk courts; @EconDesk layoffs | `Fashion` first for @FashionDesk, `Retail` then `Fashion` for stores, `Law` for courts | Business Wire via its Yahoo Finance copy (BW itself 403s; Yahoo has the UTC `<time>`), PR Newswire (Vuori releases to 2023), WWD, Retail Dive, Drapers and TheIndustry (London openings), Jing Daily and RetailBoss (China), Shinsegae newsroom (Korea), Vuori store pages and its Brandfolder image library, CourtListener search API + RECAP PDFs | Every pin carries `Vuori`, company Vuori; private, so `stocks: []`. Corporate pins at HQ, 5600 Avenida Encinas, Carlsbad. Store pins are all-day on the opening day; a store with only "opened this week/month" takes the article's day, `estimated`. One source article can back only one pin across *all* desks |
...
| Apparel brand news (Lululemon) | @RetailDesk for results, leadership, proxy fights, products, campaigns and stores (or @FashionDesk, 414, if the session can log in to it); @LawDesk suits and regulators (incl. its own tariff-refund suit), @SportDesk races (SeaWheeze) and the Olympic parade, @EconDesk job cuts, @BuildDesk distribution centres | `Finance`/`Business`/`Fashion`/`Retail` first by lane, the desk's own for the rest | corporate.lululemon.com newsroom (Chrome UA; URL stamp = UK local time; `~/media` images curl), EDGAR CIK 1397187 (8-K acceptance times, exhibit 99.1, proxy fight DFAN14A), Yahoo Finance copies of Business Wire releases (`datePublished`), CourtListener search API + storage PDFs, olympic.ca, seawheeze.com | Every pin tagged `Lululemon`, LULU note fixed. Results come ~4:05 pm ET, 39 days after quarter end; fiscal year ends the Sunday nearest 31 Jan. HQ label "1818, Cornwall Avenue, Kitsilano, Vancouver..." (49.2719955, -123.1470927). Business Wire and shop.lululemon.com are walled |
| Apparel brand news (Alo Yoga) | @FashionDesk (414) for company news, campaigns, ambassadors and collections; @SneakerDesk for each footwear release day; @RetailDesk for each store (the store is the pin); @LawDesk courts and regulators | `Fashion` first, `Retail` then `Fashion` for stores, `Law` for courts | aloyoga.com (Chrome UA: `/products/<handle>.json` prices and publish dates, blog `datePublished`), builder.io store data (all stores with coordinates), WWD, Footwear News (release days and prices), THR, Drapers/TheIndustry (London), Inside Retail/Pedestrian (Australia), CourtListener search API + RECAP PDFs, settlement sites | Every pin carries `Alo Yoga`, company Alo Yoga; private, so `stocks: []`. Corporate pins at HQ, 9830 Wilshire Blvd, Beverly Hills (34.0666372, -118.4095343) until the La Peer Building move is dated. Campaign pins are all-day on the day Alo's own YouTube film went up. A sister brand of the founders (Bella+Canvas) takes its own company and keeps the `Alo Yoga` tag |
| Apparel brand news (Nike) | @FashionDesk for results, leadership, shareholders, collaborations, campaigns and athlete deals; @SneakerDesk each shoe release day; @RetailDesk stores and channel deals (Amazon); @SportDesk races Nike stages (Breaking4, After Dark Tour) and kit debuts; @LawDesk courts and regulators; @EconDesk job cuts | `Fashion` first for @FashionDesk and @SneakerDesk, `Retail`, `Sports`, `Law`, `Labour` for the others | about.nike.com releases (Chrome UA; JSON-LD `datePublished` gives the minute; `sitemap.xml` for discovery, its lastmod is an edit date), Nasdaq `/press-release/` copies of Business Wire (4:15 pm ET stamp), EDGAR CIK 320187 (8-K, Form 4), SNKRS launch page (date + MSRP ~1 week out), afterdarktour.nike.com city pages (time + start line), CourtListener, state WARN PDFs (tn.gov, mdes.ms.gov) | Every pin tagged `Nike`, NKE note fixed. Fiscal year ends 31 May; results late Jun/Sep-Oct/Dec/Mar. HQ label is Nominatim's search result "Nike Philip H. Knight Campus, 1, Merlo Station, Marlene Village, Beaverton, ..." (45.5103135, -122.831375); sneaker drops with no venue sit at HQ, all-day. A race schedule threads newest first |
| Chipmaker news (Qualcomm) | @TechDesk results, leadership, deals, products, keynotes; @LawDesk courts and regulators (`company: Qualcomm`, or the other party when it's their move); @CyberDesk exploited-chip security bulletins; @EconDesk WARN layoffs | `Finance` + `Semiconductors` for results, `Semiconductors` + `AI`/`Telecom`/`Computing` for chips, `Conference` for summits, `Law`, `Cybersecurity`, `Labour` | qualcomm.com `<url>.md` / `.model.json` (publish stamp at `mediaheader.titleInfo.publishDate`), `sitemap.xml`, IR feeds `investor.qualcomm.com/feed/...svc` (keyless), EDGAR CIK 804328, Nasdaq `/press-release/` copies (Qualcomm Inc. only), catribunal.org.uk, CourtListener search API, CA EDD WARN PDFs | Every pin tagged `Qualcomm`, QCOM note fixed. Fiscal year ends the last Sunday of September; results 4:00 pm ET late Jul/early Nov/early Feb/late Apr, Q4 on a Wednesday ~38 days after year end. HQ label is Nominatim's search result "Qualcomm N, 5775, Morehouse Drive, ..." (32.8959414, -117.1958195). Snapdragon Summit is in Maui in late September (HST = UTC-10); launches made there sit at the summit venue |
...
| City news (New York) | As San Francisco: @CityDesk city hall and civic (inauguration, budgets, the rent freeze, 2-K, congestion pricing and fares, incidents, the parade, tree lighting and ball drop), @PoliticsDesk the votes and UNGA, @LawDesk courts and crimes, @BuildDesk builds (extending the existing Gateway, JFK, Penn Station threads), @SportDesk (marathon, World Cup at MetLife), @ClimateDesk (Climate Week) | The event's own | vote.nyc results PDFs and RCV pages, nyc.gov/mayors-office and council.nyc.gov, the Rent Guidelines Board's orders, CourtListener RECAP PDFs for SDNY rulings, manhattanda.org, NTSB investigation pages for incidents, mta.info and governor.ny.gov through the scraper, THE CITY, Gothamist, NY1, CBS New York | Every pin carries the tag `New York` (city and state alike, no `NYC` twin). City-wide actions sit at City Hall, state ones at the Capitol in Albany, a case at its own courthouse (the judge's page names the building). Long stories are one oldest-first chain each (the mayoral race, Combs, Mangione, congestion pricing). Eastern time, all-day on the local date when no time is given |
| City news (Los Angeles) | As San Francisco: @CityDesk city hall (mayor, budgets, LAFD leadership, city measures, the homeless count, a curfew), @PoliticsDesk the votes and the federal Guard orders, @LawDesk courts (Newsom v. Trump, the Palisades arson case, the Eaton suits), @ClimateDesk fires, @BuildDesk Metro, LAWA and fire rebuilding, @SportDesk (World Cup at SoFi, LA28, Rose Bowl, Marathon), @FilmDesk (Emmys, AFI Fest), @MusicDesk, @EconDesk (tourism, the Olympic wage), @TechDesk (Auto Show), @AnimeDesk (Anime Expo) | The event's own | mayor.lacity.gov and clerk.lacity.gov, metro.net releases and PDFs (not thesource.metro.net), lawa.org releases with their hour (not flylax.com), CAL FIRE incident pages, justice.gov/usao-cdca case pages, LAist, NBC LA, ABC7, KTLA, CBS LA, The LA Local, Bond Buyer, the city's ordinances for wage steps | Every pin carries the tag `Los Angeles`. A fire sits at its origin point and has no company; a court story sits at the courthouse (First Street federal, or the Ninth Circuit in San Francisco). Long stories are one oldest-first chain each (Palisades, Eaton, the Guard, the budget, the mayoral race). Pacific time, all-day on the local date when no time is given |
