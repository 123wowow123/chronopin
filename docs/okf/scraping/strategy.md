...
**Which media.** Make a best effort to reach **3 media** on every pin, **at least 1 of them a picture**: work down the sources in order and do not stop at the first one that adds a single picture. The page's own picture stays the default heading and the rest follow it. A video (embedded on the page, or a trailer) is a medium too, goes after the pictures, and its still is added as a picture when the page had none, so a video-only page still ends with a picture. If every source is exhausted with fewer than 3, save what was found and record in the job what was tried; never pad with a URL that was not fetched, an unrelated stock picture or a duplicate.
...
- 3 media reached by best effort, with at least 1 picture among them (pictures first, then video); fewer only when every source was tried, with what was tried noted.
