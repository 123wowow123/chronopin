...
**By hand:** `og:image` from the source or a Wikipedia article (decode `&amp;`); download through the app's medium code, not the raw CDN, and space Wikimedia downloads out (about 4 seconds, with backoff) because its CDN answers 429 after a few in a row.
...
- **Backfill:** `npm run media:screen` (dry run by default, `--apply`, `--ids`, `--skip-trailer`).
