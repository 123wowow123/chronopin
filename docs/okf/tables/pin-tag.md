...
type: Postgres Table
title: PinTag
description: A pin's tags - its categories, those typed in the form, or awards its own text names - plus PinTagView, which adds a tag for every award body and year its work won, a Nominee tag where it was only nominated, and the site's own reserved filters.
...
| `kind` | varchar(10) | `category` (a name on the list in src/lib/categories.ts), `award`, `nomination` (a name ending in Nominee) or `topic`, from the name (`tagKind` in src/lib/tags.ts). `reserved` is the view's own (0063) and is never a row: a reserved name is dropped on the way in |
...
Some filters read as tags without being anyone's: the `Thread` tag a pin in a chain carries (0056, derived from a fact about two pins), and how well a pin is dated and evidenced - the `confidence:` levels (`Confirmed`, `Scheduled`, `Estimated`, `Delayed`, `Unverified`) and score bands (`High confidence`, `Medium confidence`, `Low confidence`), which are a column and a score read off PinReference, no rows at all. `RESERVED_TAGS` in src/lib/tags.ts lists them with the term each one writes, and their labels are the site's words, read in the page's language (`reserved.*` messages, `tagLabel`).

0063 gives the derived `Thread` row kind `reserved`, and `PinTag` drops a reserved name on the way in, so nobody can tag a pin `Thread` or `Estimated` and have it mean something else. The names as rows were deleted with that schema.

They lead the tag cloud in a strip of their own - in their own order rather than by count, outlined and locked rather than sized - and never stand among the words people typed: `tag-counts` answers them first, with kind `reserved`, counted outside `limit` so the cloud offers the same few whatever else is busy. The search box suggests them among the Tags, outlined the same way and ending in the term they write (`confidence:estimated`) rather than a count; those are matched in the route (`reservedSuggestions`), since most are no rows to count. A pin page's chips mark them the same way.
